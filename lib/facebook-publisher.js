class FacebookPublisher {
    ...
    async createAdCreative(imageUrl, linkUrl, linkName, caption, description) {
        ...
        try {
            ...
            console.log(`  📋 Creative ID: ${response.id}`);
            return response.id;
        } catch (error) {
    ...
    async waitForPostId(creativeId, maxAttempts = 10) {
        ...
        console.log(`  ⏳ This may take up to ${maxAttempts * 3} seconds...`);
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if (response.effective_object_story_id) {
                    console.log(`✅ Post ID generated successfully!`);
                    console.log(`  🆔 Post ID: ${response.effective_object_story_id}`);
                    return response.effective_object_story_id;
                }
                
                if (attempt < maxAttempts) {
                    console.log(`  ⏱️ Post ID not ready yet, waiting 3 seconds...`);
    ...
    async publishPost(postId) {
        ...
        console.log(`  🔑 Using secondary access token for publishing...`);
